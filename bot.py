...
# ============================================================
# SERVEUR WEB — DISCLOUD / REPLIT / HEROKU / ECLOUDSERV
# ============================================================
...
def est_staff(interaction: discord.Interaction):
    ...
    return (
        ...
        or ROLE_ADMIN_ID in roles_ids
        or ROLE_MODERATEUR_ID in roles_ids
    )
...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel is None and after.channel is not None:
        vocal_sessions[member.id] = now
    elif before.channel is not None and after.channel is None:
        start_time = vocal_sessions.pop(member.id, None)
        if start_time:
            duree = int(now - start_time)
            temps_voc = donnees.setdefault("temps_vocal", {})
            temps_voc[user_id_str] = temps_voc.get(user_id_str, 0) + duree
            sauvegarder()
...
async def liste_commandes(interaction: discord.Interaction):
    embed = discord.Embed(
        title="📜 Liste des Commandes — Aethoria Bot",
        description="Voici l'ensemble des commandes accessibles aux joueurs :",
        color=0x3498DB,
    )
    embed.add_field(
        name="🏰 Salons Permanents (`/salon`)",
        value=(
            "• `/salon créer <nom>` : Créer tes propres salons vocal et textuel.\n"
            "• `/salon supprimer` : Supprimer tes salons permanents.\n"
            "• `/salon ajouter joueur @membre` : Donner l'accès à tes salons.\n"
            "• `/salon retirer joueur @membre` : Retirer l'accès à tes salons."
        ),
        inline=False,
    )
    embed.add_field(
    ...
    embed.add_field(
        name="🎫 Support",
        value="• Utilise le panneau interactif dans le salon support pour ouvrir un ticket.",
        inline=False,
    )
    embed.set_footer(text="Aethoria Bot • Projets & Communauté")

    await interaction.response.send_message(embed=embed, ephemeral=True)

@tree.command(name="top_voc", description="Afficher le classement des personnes les plus actives en vocal")
async def top_voc(interaction: discord.Interaction):
    ...
    for i, (user_id, secondes) in enumerate(trie):
...
async def obtenir_stats_minecraft(ip):
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api.mcsrvstat.us/2/{ip}", timeout=5) as resp:
                if resp.status == 200:
...
async def update_tableau_de_board():
    ...
    java_online, java_players, java_max = await obtenir_stats_minecraft(SERVER_IP_JAVA)
    java_statut = "🟢" if java_online else "🔴"

    bedrock_online, bedrock_players, bedrock_max = await obtenir_stats_minecraft(SERVER_IP_BEDROCK)
    ...
    texte_evolution = "\n".join(lignes_evolution) if lignes_evolution else f"{date_aujourdhui} : {total_membres}"
...
@tree.command(name="setup_tableau_de_bord", description="[STAFF] Initialiser le message du tableau de bord")
async def setup_tableau_de_bord(interaction: discord.Interaction):
    if not peut_gerer(interaction):
        await interaction.response.send_message(
            "❌ Permission insuffisante. Seul le Staff Bot peut exécuter cette commande.",
            ephemeral=True,
        )
        return
...
# ============================================================
# COMMANDES EMBED & GESTION DU STAFF
# ============================================================

@tree.command(name="embed", description="[STAFF] Créer et envoyer un message Embed personnalisé")
@app_commands.describe(
    titre="Titre de l'embed",
    description="Contenu / Description de l'embed",
    couleur_hex="Couleur au format Hex (ex: 3498db ou #3498db)",
    image_url="Lien direct vers une image à intégrer (Optionnel)",
)
async def creer_embed(
    interaction: discord.Interaction,
    titre: str,
    description: str,
    couleur_hex: str = "3498db",
    image_url: str = None,
):
    if not est_staff(interaction):
        await interaction.response.send_message(
            "❌ Cette commande est réservée aux membres du staff.",
            ephemeral=True,
        )
        return

    clean_hex = couleur_hex.lstrip("#")
    try:
    ...
    embed.set_footer(text=f"Posté par {interaction.user.display_name}")

    await interaction.channel.send(embed=embed)
    await interaction.response.send_message("✅ Embed envoyé avec succès !", ephemeral=True)

groupe_ajouter_main = app_commands.Group(name="ajouter", description="Ajouter des rôles ou accès")

@groupe_ajouter_main.command(name="staff_bot", description="[ADMIN] Définir un joueur comme Staff Bot")
@app_commands.describe(joueur="Le joueur à passer Staff")
async def ajouter_staff_bot(interaction: discord.Interaction, joueur: discord.Member):
    ...
    role_staff = interaction.guild.get_role(ROLE_MODERATEUR_ID)
    if not role_staff:
        await interaction.response.send_message("❌ Le rôle Staff/Modérateur est introuvable.", ephemeral=True)
        return

    await joueur.add_roles(role_staff)
    await interaction.response.send_message(f"✅ {joueur.mention} a été ajouté à l'équipe Staff du Bot !", ephemeral=True)

groupe_retirer_main = app_commands.Group(name="retirer", description="Retirer des rôles ou accès")

@groupe_retirer_main.command(name="staff_bot", description="[ADMIN] Retirer un joueur de l'équipe Staff Bot")
@app_commands.describe(joueur="Le joueur à retirer de l'équipe Staff")
async def retirer_staff_bot(interaction: discord.Interaction, joueur: discord.Member):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message(
            "❌ Seuls les Administrateurs du serveur peuvent exécuter cette commande.",
            ephemeral=True,
    ...
    await joueur.remove_roles(role_staff)
    await interaction.response.send_message(f"🚫 {joueur.mention} n'est plus membre de l'équipe Staff du Bot.", ephemeral=True)

tree.add_command(groupe_ajouter_main)
tree.add_command(groupe_retirer_main)
...
class MenuChoixTicket(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="Problème Jeu — Java", description="Un souci ou bug sur la version Java (PC)", emoji="💻", value="java"),
            discord.SelectOption(label="Problème Jeu — Bedrock", description="Un souci ou bug sur Bedrock", emoji="📱", value="bedrock"),
            discord.SelectOption(label="Signalement", description="Signaler un joueur ou un comportement", emoji="⚠️", value="signalement"),
            discord.SelectOption(label="Problème avec un Staff", description="Ticket privé géré UNIQUEMENT par l'Administration", emoji="🛡️", value="staff_problem"),
            discord.SelectOption(label="Contestation de Sanction", description="Contester un ban, mute ou warn", emoji="🔨", value="sanction"),
            discord.SelectOption(label="Recrutement", description="Postuler pour l'équipe du staff", emoji="📝", value="recrutement"),
            discord.SelectOption(label="Notation", description="Laisser une note ou un avis sur le serveur", emoji="⭐", value="notation"),
            discord.SelectOption(label="Autre", description="Autre demande spécifique", emoji="📩", value="autre"),
        ]
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not log_channel:
            await interaction.followup.send("❌ Le salon des logs est introuvable.", ephemeral=True)
            return
        ...
        embed_log.add_field(name="👤 Demandeur", value=f"{membre.mention} (`{membre.id}`)", inline=True)
        embed_log.add_field(name="🏷️ Type de Ticket", value=f"`{type_ticket.upper()}`", inline=True)
        embed_log.set_footer(text="En attente de validation par l'équipe...")

        view_valider = ValiderTicketView(membre_id=membre.id, type_ticket=type_ticket)
...
class ValiderTicketView(discord.ui.View):
    ...
    @discord.ui.button(label="Accepter", style=discord.ButtonStyle.success, emoji="✅", custom_id="bouton_accepter_ticket")
    async def accepter(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not peut_gerer(interaction):
            await interaction.response.send_message("❌ Seul le staff peut valider les tickets.", ephemeral=True)
            return
        ...
        if not self.membre_id and embed_log.fields:
            user_id_str = embed_log.fields[0].value.split("`")[1] if "`" in embed_log.fields[0].value else "0"
            self.membre_id = int(user_id_str)
            self.type_ticket = embed_log.fields[1].value.replace("`", "").lower()

        membre = guild.get_member(self.membre_id)
        if not membre:
            await interaction.followup.send("❌ Le joueur est introuvable ou a quitté le serveur.", ephemeral=True)
            return
        ...
        permissions = {
            ...
            membre: discord.PermissionOverwrite(read_messages=True, send_messages=True, read_message_history=True),
        ...
        if self.type_ticket in ["staff_problem", "sanction"]:
            if role_modo:
                permissions[role_modo] = discord.PermissionOverwrite(read_messages=False)
            if role_admin:
                permissions[role_admin] = discord.PermissionOverwrite(read_messages=True, send_messages=True, read_message_history=True)
        else:
            if role_modo:
                permissions[role_modo] = discord.PermissionOverwrite(read_messages=True, send_messages=True, read_message_history=True)
            if role_admin:
                permissions[role_admin] = discord.PermissionOverwrite(read_messages=True, send_messages=True, read_message_history=True)

        salon_ticket = await guild.create_text_channel(name=nom_salon, category=categorie, overwrites=permissions)

        date_ouverture = datetime.now(timezone.utc)
        donnees.setdefault("tickets_config", {}).setdefault("tickets_actifs", {})[str(salon_ticket.id)] = {
            "owner_id": membre.id,
        ...
        embed_log.set_footer(text=f"✅ Ticket accepté par {interaction.user.display_name} -> #{nom_salon}")
        await interaction.message.edit(embed=embed_log, view=self)

        embed_ouvert = discord.Embed(title="📥 Ticket Ouvert", color=0x2ECC71)
        embed_ouvert.add_field(name="👤 Auteur", value=f"{membre.mention} (`{membre.id}`)", inline=True)
        embed_ouvert.add_field(name="🏷️ Type", value=f"`{self.type_ticket.upper()}`", inline=True)
        embed_ouvert.add_field(name="💬 Salon", value=salon_ticket.mention, inline=False)
        ...
        await interaction.followup.send(f"✅ Ticket accepté ! Salon créé : {salon_ticket.mention}", ephemeral=True)

    @discord.ui.button(label="Refuser", style=discord.ButtonStyle.danger, emoji="❌", custom_id="bouton_refuser_ticket")
    async def refuser(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not peut_gerer(interaction):
            await interaction.response.send_message("❌ Seul le staff peut refuser les tickets.", ephemeral=True)
            return
        ...
        if membre:
            try:
                await membre.send(f"❌ Ta demande de ticket `{self.type_ticket}` sur **{interaction.guild.name}** a été refusée par l'équipe.")
            except Exception:
                pass
        ...
        embed_log.set_footer(text=f"❌ Ticket refusé par {interaction.user.display_name}")
        await interaction.message.edit(embed=embed_log, view=self)
...
class FermerTicketView(discord.ui.View):
    ...
    @discord.ui.button(label="Fermer le ticket", style=discord.ButtonStyle.danger, emoji="🔒", custom_id="bouton_fermer_ticket")
    async def fermer_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        membre_auteur = interaction.guild.get_member(owner_id) if owner_id else None
        auteur_mention = f"{membre_auteur.mention} (`{owner_id}`)" if membre_auteur else (f"`{owner_id}`" if owner_id else "Inconnu")
        ferme_par_mention = f"{interaction.user.mention} (`{interaction.user.id}`)"
        ...
        embed_fermeture = discord.Embed(title="🔒 Ticket Fermé", color=0xE74C3C)
        embed_fermeture.add_field(name="👤 Auteur du ticket", value=auteur_mention, inline=True)
        embed_fermeture.add_field(name="🛠️ Fermé par", value=ferme_par_mention, inline=True)
        embed_fermeture.add_field(name="🏷️ Type", value=f"`{type_ticket}`", inline=False)
        embed_fermeture.add_field(name="🕒 Date ouverture", value=date_ouverture_str, inline=True)
        embed_fermeture.add_field(name="⌛ Date fermeture", value=date_fermeture_str, inline=True)
        embed_fermeture.set_footer(text=f"Salon : {interaction.channel.name}")
...
@tree.command(name="setup_logs_tickets", description="[STAFF] Définir le salon de réception des demandes de tickets")
@app_commands.describe(salon="Le salon textuel de logs des tickets")
async def setup_logs_tickets(interaction: discord.Interaction, salon: discord.TextChannel):
    if not peut_gerer(interaction):
        await interaction.response.send_message("❌ Permission insuffisante. Réservé au Staff Bot.", ephemeral=True)
        return

    donnees.setdefault("tickets_config", {})["channel_logs_id"] = salon.id
    sauvegarder()

    await interaction.response.send_message(f"✅ Salon des logs de tickets configuré sur {salon.mention}.", ephemeral=True)

@tree.command(name="setup_panneau_tickets", description="[STAFF] Envoyer le message de création des tickets")
...
async def salon_creer(interaction: discord.Interaction, nom: str):
    ...
    overwrites = {
        ...
        membre: discord.PermissionOverwrite(read_messages=True, send_messages=True, connect=True, speak=True, manage_channels=True),
        guild.me: discord.PermissionOverwrite(read_messages=True, send_messages=True, connect=True, manage_channels=True),
    }

    salon_texte = await guild.create_text_channel(name=f"💬-{nom_clean}", category=categorie, overwrites=overwrites)
...
async def ajouter_joueur(interaction: discord.Interaction, joueur: discord.Member):
    ...
    for chan in salons_perso:
        if isinstance(chan, discord.TextChannel):
            await chan.set_permissions(joueur, read_messages=True, send_messages=True)
...
async def on_message(message):
    ...
    if message.channel.id == CHANNEL_JEU_ID:
        ...
        if jeu.get("dernier_joueur_id") == message.author.id:
            ...
            msg_err = await message.channel.send(f"❌ {message.author.mention}, tu ne peux pas jouer deux fois d'affilée !")
        ...
        if len(mots_msg) > LIMITE_MOTS:
            ...
            msg_err = await message.channel.send(f"❌ {message.author.mention}, ton message dépasse la limite de {LIMITE_MOTS} mot(s) !")
            await asyncio.sleep(4)
            try:
                await msg_err.delete()
            except Exception:
